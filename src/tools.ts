/**
 * The ClinicDesk tool surface — the whole server minus the transport.
 *
 * This file knows nothing about where it runs. It registers the eleven tools
 * onto any McpServer and reads its credentials from a plain object, so the same
 * clinical logic serves the Cloudflare Worker (src/index.ts, Durable Object
 * session per client) and the Vercel function (api/mcp.ts, stateless). Porting
 * between hosts must never mean re-testing the triage.
 *
 * Design note that drives everything below: the caller is speaking, not typing.
 * A patient at 2am with a bleeding socket cannot read JSON and cannot type. So
 * every tool returns a short `speech` line meant to be read aloud verbatim,
